  ...
  .handler(async ({ data }) => {
    ...
    return {
      ...
      product: product ? {
        ...
        source: (product.source as string | null) ?? null,
        delivery_type: (product.delivery_type as string | null) ?? null,
        price_try: num(product.price_try) ?? 0,
      } : null,
      keys: keys.map((k) => ({ delivered_at: k.delivered_at ? String(k.delivered_at) : null, key: { id: String(k.id), key_value: String(k.key_value), status: String(k.status), expires_at: k.expires_at ? String(k.expires_at) : null, hwid: (k.hwid as string | null) ?? null, activated_at: k.activated_at ? String(k.activated_at) : null, duration_days: num(k.duration_days) } })),
