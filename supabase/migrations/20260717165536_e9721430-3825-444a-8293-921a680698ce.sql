...
BEGIN
  ...
  -- Aynı kullanıcı aynı kodu daha önce (pending olmayan bir siparişte) kullanmışsa reddet
  ...
  WHERE o.user_id = v_uid
    ...
    AND upper(od.code_snapshot) = v_norm;

  IF v_already_used > 0 THEN
    RAISE EXCEPTION 'kod_zaten_kullanildi';
  END IF;

  SELECT * INTO v_pc
  FROM public.promo_codes pc
  WHERE upper(pc.code) = v_norm
  LIMIT 1;
  ...
  IF v_source = 'promo' THEN
    UPDATE public.promo_codes pc SET used_count = pc.used_count + 1 WHERE pc.id = v_promo_id;
  ELSE
    UPDATE public.coupons cp SET used_count = cp.used_count + 1 WHERE upper(cp.code) = v_code_str;
  END IF;
  ...
  apply_promo_code.discount_try := v_discount;
  apply_promo_code.final_price := GREATEST(0, v_order.price_try - v_total_discount);
  apply_promo_code.code := v_code_str;
  RETURN NEXT;
END;
$function$;