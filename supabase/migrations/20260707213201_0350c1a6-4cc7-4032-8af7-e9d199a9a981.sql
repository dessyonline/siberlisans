...
BEGIN
  ...
  -- Yoksa otomatik SIBER-XXXX-XXXX-XXXX üret (manuel/sınırsız dahil her ürün için)
...
-- Zaten onaylı ama key atanmamış eski siparişlere de otomatik SIBER key ata
DO $$
DECLARE
  r RECORD;
  v_chars TEXT := 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  v_seg TEXT; v_new_key TEXT; v_key_id UUID; v_attempt INT; v_i INT; v_j INT;
BEGIN
  FOR r IN
    SELECT o.id AS order_id, o.product_id
    FROM public.orders o
    LEFT JOIN public.order_keys ok ON ok.order_id = o.id
    WHERE o.status = 'approved' AND ok.id IS NULL
  LOOP
    v_attempt := 0;
    LOOP
      v_attempt := v_attempt + 1;
      ...
      END LOOP;
      BEGIN
        INSERT INTO public.license_keys (product_id, key_value, status, assigned_order_id, assigned_at)
        VALUES (r.product_id, v_new_key, 'assigned', r.order_id, now())
        RETURNING id INTO v_key_id;
        INSERT INTO public.order_keys (order_id, license_key_id) VALUES (r.order_id, v_key_id);
        EXIT;
      EXCEPTION WHEN unique_violation THEN
        IF v_attempt > 12 THEN EXIT; END IF;
      END;
    END LOOP;
  END LOOP;
END $$;
