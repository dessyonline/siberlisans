...
BEGIN
  ...
  SELECT o.product_id, o.status, p.name, p.delivery_type, p.duration, p.default_license_days, p.unlimited_stock
    INTO v_product_id, v_status, v_product_name, v_delivery, v_duration, v_default_days, v_unlimited
  ...
  IF v_key_id IS NULL THEN
    RAISE EXCEPTION 'Havuzda "%" ürünü için kullanılabilir anahtar yok. Key havuzuna gerçek anahtarı ekleyip tekrar onaylayın (otomatik sahte anahtar üretilmez).', COALESCE(v_product_name, 'ürün');
  END IF;
