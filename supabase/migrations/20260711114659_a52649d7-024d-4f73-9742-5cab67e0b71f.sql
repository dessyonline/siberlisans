...
BEGIN
  ...
  IF v_row.hwid IS NULL THEN RETURN jsonb_build_object('valid', false, 'error', 'Lisans henüz etkinleştirilmemiş.'); END IF;
  IF v_row.hwid <> v_hwid THEN RETURN jsonb_build_object('valid', false, 'error', 'Bu lisans başka bir cihaza kilitli.'); END IF;
...
DECLARE
  v_row public.license_keys%ROWTYPE;
  v_minutes INTEGER; v_key TEXT; v_hwid TEXT;
  v_email TEXT; v_name TEXT; v_email_masked TEXT;
BEGIN
  v_key := upper(btrim(coalesce(_key,'')));
  v_hwid := btrim(coalesce(_hwid,''));
  ...
  IF v_row.id IS NULL THEN RETURN jsonb_build_object('success', false, 'error', 'Lisans anahtarı bulunamadı.'); END IF;
  IF v_row.revoked THEN RETURN jsonb_build_object('success', false, 'error', 'Bu lisans iptal edilmiş.'); END IF;
  ...
  IF v_row.hwid IS NULL THEN
    UPDATE public.license_keys
      SET hwid = v_hwid, activated_at = COALESCE(activated_at, now()),
